...
///                  v_r / v_phi / H_r / H_phi.
///
///   HPhi_r0_over_r Free-vortex (no-azimuthal-current) inlet profile for H_phi:
///                    H_phi = r_0 / r
///                  where r_0 = (r_inner(0) + r_outer(0)) / 2 is the mid-channel
///                  radius at the inlet and r = grid.r[l][m] is the local cell
///                  radius.  This enforces H_phi · r = r_0 = const across the
///                  entire inlet face, which is the correct current-free
///                  (force-free azimuthal field) profile.
///
///                  Valid only for H_phi.  Physically meaningful on L_LO / L_HI
///                  faces (inlet / outlet).
