...
void IOManager::unpack_into_global(Array2D& dst,
...
void IOManager::gather_global(const Fields& f, const Grid& grid) {
    ...
    const Array2D* local_fields[nfields] = {
        &f.rho, &f.v_z, &f.v_r, &f.v_phi, &f.e,
        &f.H_z, &f.H_r, &f.H_phi, &grid.r
    };

    Array2D* global_fields[nfields] = {
    ...
    if (mpi_.rank != 0) {
        ...
        for (int fi = 0; fi < nfields; ++fi) {
            ...
            MPI_Send(buf.data(), static_cast<int>(buf.size()),
                     MPI_DOUBLE, 0, fi, MPI_COMM_WORLD);
        }
        return;
    }
    ...
    for (int src = 1; src < mpi_.size; ++src) {
        ...
        for (int fi = 0; fi < nfields; ++fi) {
            MPI_Recv(recv_buf.data(), buf_size, MPI_DOUBLE,
                     src, fi, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void IOManager::write_vtk(const std::string& filepath) const {
    ...
    if (writer->GetErrorCode() != 0)
        std::cerr << "Warning: VTK writer reported an error for "
                  << filepath << "\n";
}
