...
auto main(int argc, char* argv[]) -> int {
    ...
    while (t < params.T && !converged) {
        ...
        // Update central part
        update_physical_fields(fields, u, grid, domain.local_L + 2, params.M_max, params.gamma);

        // Apply boundary conditions
        apply_boundary_conditions(fields, u, grid, domain, params, r_0);

        // Data update
        update_physical_fields(fields, u, grid, domain.local_L_with_ghosts, params.M_max, params.gamma);

        // Copy u to u0
        copy_conservative_vars(u0, u, domain.local_L_with_ghosts, params.M_max);

        // Convergence check
        if (params.convergence_threshold > 0 && step_count % params.check_frequency == 0) {
            double change = compute_solution_change(
                fields.rho, prev_state.rho_prev, 
                fields.v_z, prev_state.v_z_prev, 
                fields.v_r, prev_state.v_r_prev,
                fields.v_phi, prev_state.v_phi_prev, 
                fields.H_z, prev_state.H_z_prev, 
                fields.H_r, prev_state.H_r_prev,
                fields.H_phi, prev_state.H_phi_prev, 
                domain.local_L, params.M_max
            );
        ...
        if (step_count % 100 == 0) {
            double max_wave_speed = compute_max_wave_speed(
                fields.rho, fields.v_z, fields.v_r, fields.v_phi, 
                fields.H_z, fields.H_r, fields.H_phi, fields.p,
                domain.local_L, params.M_max, params.gamma
            );
    ...
    if (domain.rank == 0) {
        ...
        memory_allocation_2D(global_grid.r, params.L_max_global + 1, params.M_max + 1);
    }
    ...
    if (domain.rank == 0) {
        ...
        if (params.output_format == "vtk") {
            WriteVtk("output_MHD.vtk", params.L_max_global, params.M_max, params.dz,
                    global_grid.r, global_fields.rho, global_fields.v_z, global_fields.v_r,
                    global_fields.v_phi, global_fields.e, global_fields.H_z, 
                    global_fields.H_r, global_fields.H_phi);
        }
