...
void ComputeTimeStep2D(ConservativeVars& u, const ConservativeVars& u0,
    ...
    // Note: In 2D decomposition, we iterate over local indices [1, local_L] x [1, local_M]
    // The stencil accesses l±1 and m±1, which are covered by ghost cells
    #pragma omp parallel for collapse(2)
    for (int l = 1; l <= local_L; l++) {
        for (int m = 1; m <= local_M; m++) {
...
void UpdatePhysicalFields2D(PhysicalFields& fields, const ConservativeVars& u,
    ...
    for (int l = 1; l < local_L_with_ghosts - 1; l++) {
        for (int m = 1; m < local_M_with_ghosts - 1; m++) {
