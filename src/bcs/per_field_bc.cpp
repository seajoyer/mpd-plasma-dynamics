...
PerFieldBC::PerFieldBC(enum FaceBC::Face face, const BCSegmentConfig& seg)
    ...
    const bool wall_tangent_used = seg.v_r.type == FieldCondType::WallTangent ||
                                   seg.H_r.type == FieldCondType::WallTangent;
    if (wall_tangent_used && face_ != FaceBC::Face::M_LO && face_ != FaceBC::Face::M_HI) {
        throw std::runtime_error(
            "PerFieldBC: WallTangent condition is only valid on M_LO / M_HI faces");
    }

    // Validate HPhi_r0_over_r is only applied to H_phi.
    const bool hphi_r0_on_other = seg.rho.type == FieldCondType::HPhi_r0_over_r ||
...
void PerFieldBC::Apply(BCContext& ctx) const {
    ...
    const bool is_l_face = (face_ == FaceBC::Face::L_LO || face_ == FaceBC::Face::L_HI);
    ...
    auto resolve = [](const FieldCond& c, double cell, double nb) -> double {
        switch (c.type) {
            case FieldCondType::Neumann:
                return nb;
            case FieldCondType::Dirichlet:
                return c.value;
            default:
                return cell;  // handled externally
        }
    };
    ...
    for (int l = ctx.local_lo; l <= ctx.local_hi; ++l) {
        ...
        if (has_axis_lf_) {
            if (rho_.type == FieldCondType::AxisLF) {
                f.u_1[l][m] = AxisLfU1(f, g, l, m, dt, dz);
            }
            if (v_z_.type == FieldCondType::AxisLF) {
                f.u_2[l][m] = AxisLfU2(f, g, l, m, dt, dz);
            }
            if (e_.type == FieldCondType::AxisLF) {
                f.u_5[l][m] = AxisLfU5(f, g, l, m, dt, dz);
            }
            if (H_z_.type == FieldCondType::AxisLF) {
                f.u_7[l][m] = AxisLfU7(f, g, l, m, dt, dz);
            }
